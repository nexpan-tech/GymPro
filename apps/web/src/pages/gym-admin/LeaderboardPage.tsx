      ...
      {loading ? <Skeleton height="h-64" /> : (
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            ...
            <Stat label="Reward redemption" value={`${analytics?.rewardRedemptionRate ?? 0}%`} icon={<Gift className="h-5 w-5" />} tone="flame" />
            <Stat label="Referral conversion" value={`${analytics?.referralConversionRate ?? 0}%`} icon={<Users className="h-5 w-5" />} tone="neutral" />
            <Stat label="Avg level" value={String(analytics?.avgLevel ?? 1)} icon={<Flame className="h-5 w-5" />} tone="steel" />
          </div>
                ...
                {rows.map((r) => (
                  <div key={r.memberId} className="flex items-center justify-between px-5 py-3 text-sm">
                    <span className="flex items-center gap-3">
                      <span className={`w-7 text-center font-bold ${r.rank <= 3 ? "text-muted-foreground" : "text-(--text-secondary)"}`}>
                    ...
                    <span className="flex items-center gap-3">
                      ...
                      <span className="flex items-center gap-1 text-(--text-secondary)"><Flame className="h-3 w-3 text-muted-foreground" />{r.streak ?? 0}</span>
                      <span className="font-bold text-primary">{r.xp} pts</span>
                    </span>
                  </div>
                ))}
              </div>
