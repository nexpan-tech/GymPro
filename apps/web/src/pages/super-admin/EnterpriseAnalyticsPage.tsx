    ...
    <Page title="Enterprise Analytics" description="Platform-wide rollup across billing, retention, engagement, and communication.">
      <div className="space-y-6">
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <Stat label="ARR" value={inr(data?.revenue.arr ?? 0)} icon={<IndianRupee className="h-5 w-5" />} tone="emerald" />
          ...
          <Stat label="Retention" value={`${data?.retention.retentionRate ?? 0}%`} icon={<HeartPulse className="h-5 w-5" />} tone="emerald" />
          <Stat label="Churn" value={`${data?.retention.churnRate ?? 0}%`} icon={<TrendingDown className="h-5 w-5" />} tone="rose" />
          <Stat label="Engagement (challenges)" value={String(data?.engagement.challengeParticipations ?? 0)} icon={<Trophy className="h-5 w-5" />} tone="amber" />
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data?.revenue.revenueTrend ?? []}>
                <defs><linearGradient id="rev" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#6366f1" stopOpacity={0.4} /><stop offset="95%" stopColor="#6366f1" stopOpacity={0} /></linearGradient></defs>
